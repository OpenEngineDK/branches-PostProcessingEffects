...
SimpleDoF::SimpleDoF(Viewport* viewport) : PostProcessingEffect(viewport,true) {
    ...
    zfarblur.push_back(3000);
}

void SimpleDoF::Setup() {
...
*/
    // dof
    IPostProcessingPass* pass3 = AddPass("Effects/SimpleDoF.frag");
    pass3->BindFloat("znear", znear);
    pass3->BindFloat("zfar", zfar);
    pass3->BindFloat("zsharp", zsharp);
    pass3->BindFloat("zfarblur", zfarblur);
...
void SimpleDoF::PerFrame(const float deltaTime) {
...
// calc gaussian values (with variance=1) in the interval [-4;4];
void SimpleDoF::calcBlurWeightArray() {
